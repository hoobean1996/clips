...
func (ecmc *EntClipMetadataCreate) SetThumbnail(s string) *EntClipMetadataCreate {
	ecmc.mutation.SetThumbnail(s)
	return ecmc
}
...
func (ecmc *EntClipMetadataCreate) check() error {
	...
	if _, ok := ecmc.mutation.Thumbnail(); !ok {
		return &ValidationError{Name: "thumbnail", err: errors.New(`ent: missing required field "EntClipMetadata.thumbnail"`)}
	}
	return nil
}
...
func (ecmc *EntClipMetadataCreate) createSpec() (*EntClipMetadata, *sqlgraph.CreateSpec) {
	...
	if value, ok := ecmc.mutation.Thumbnail(); ok {
